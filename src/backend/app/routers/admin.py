...
from app.schemas import ImportBatchRequest, ApiResponse, ImportMemberRequest
from app.services import MemberService, AvatarService
from app.crypto import decrypt_uin
    ...
    try:
        ...
        for member_data in batch_request.members:
            try:
                ...
                # 复制头像文件
                try:
                    AvatarService.copy_avatar_file(member_data.uin, member.id)
...
async def decrypt_member_uin(
    ...
    try:
        decrypted_uin = decrypt_uin(member.uin_encrypted)
        return {
            "member_id": member_id,

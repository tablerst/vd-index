...
from app.models import Member, get_session
from app.schemas import MemberResponse, MemberDetailResponse, ImportMemberRequest
from app.crypto import encrypt_uin, generate_avatar_hash, generate_secure_filename
...
class MemberService:
    ...
    def import_member_from_json(session: Session, member_data: ImportMemberRequest) -> Member:
        """从JSON数据导入成员"""
        # 加密UIN
        encrypted_uin = encrypt_uin(member_data.uin)
        ...
        # 生成头像哈希
        avatar_hash = generate_avatar_hash(member.id)
...
class AvatarService:
    ...
    @staticmethod
    def copy_avatar_file(original_uin: int, surrogate_id: int) -> str:
        """复制并重命名头像文件"""
        ...
        # 生成安全的文件名
        secure_filename = generate_secure_filename(original_uin, surrogate_id)
